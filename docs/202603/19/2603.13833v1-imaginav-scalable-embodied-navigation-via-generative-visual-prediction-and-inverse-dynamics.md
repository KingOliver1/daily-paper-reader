...
tags: ["query:robot"]
score: 7.0
evidence: 具身智能体的视觉规划与导航
tldr: ImagiNav 提出了一种解耦视觉规划与机器人执行的模块化导航框架。它利用视觉语言模型分解指令，通过生成式视频模型预测未来轨迹，并利用逆动力学模型提取动作。该方法通过自动标注的互联网视频进行训练，无需机器人演示即可实现强大的零样本迁移能力，为利用海量无标签数据训练通用导航机器人提供了新路径。
selection_source: fresh_fetch
motivation: 传统的视觉语言导航依赖昂贵的机器人特定数据，且仿真环境的多样性和真实感有限，难以扩展到开放世界。
method: 采用分层架构，结合视觉语言模型分解子目标、生成式视频模型预测轨迹以及逆动力学模型提取动作。
result: ImagiNav 在无需机器人演示数据的情况下，实现了从互联网视频到真实机器人导航的强大零样本迁移。
conclusion: 该研究证明了通过解耦规划与执行并利用大规模野外视频数据，可以有效提升具身智能体的泛化导航能力。
---
