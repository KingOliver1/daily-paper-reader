...
title_zh: IndoorR2X：基于大语言模型驱动规划的室内机器人到万物（R2X）协同
...
## 摘要
尽管机器人对机器人（R2R）通信提升了室内场景理解能力，超越了单机器人的局限，但仅靠 R2R 在不增加大量探索开销或扩大团队规模的情况下，无法克服部分可观测性问题。相比之下，许多室内环境已经包含了低成本的物联网（IoT）传感器（如摄像头），它们能提供超越机载感知的、覆盖整个建筑的持久上下文信息。因此，我们推出了 IndoorR2X，这是首个针对室内环境中具有机器人到万物（R2X）感知与通信的大语言模型（LLM）驱动多机器人任务规划的基准测试和仿真框架。IndoorR2X 整合了移动机器人和静态 IoT 设备的观测结果，以构建全局语义状态，支持可扩展的场景理解，减少冗余探索，并通过基于 LLM 的规划实现高层协同。IndoorR2X 提供了可配置的仿真环境、传感器布局、机器人团队和任务套件，用于系统地评估高层语义协同策略。在多种设置下的广泛实验表明，IoT 增强的世界建模提高了多机器人的效率和可靠性，我们还强调了推进机器人团队与室内 IoT 传感器之间基于 LLM 协作的关键见解和失败模式。
