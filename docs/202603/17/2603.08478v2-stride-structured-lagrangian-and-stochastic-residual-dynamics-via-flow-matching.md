...
tldr: STRIDE是一个结合物理结构与随机建模的动力学学习框架，旨在解决机器人系统在非结构化环境中的不确定性问题。它利用拉格朗日神经网络（LNN）建模保守的刚体动力学以保持物理一致性，并结合条件流匹配（CFM）捕捉多模态的非保守交互效应。该框架在四足和人型机器人实验中显著降低了长时预测和接触力预测误差，为复杂环境下的模型预测控制提供了更可靠的基础。
selection_source: fresh_fetch
motivation: 传统分析模型难以捕捉复杂的交互效应，而纯数据驱动模型往往违背物理规律且容易产生长时漂移。
method: 提出STRIDE框架，将动力学显式分解为基于LNN的结构化保守部分和基于CFM的随机残差交互部分，并进行端到端联合训练。
result: "在单摆、Go1四足机器人及G1人型机器人上的实验表明，长时预测误差降低了20%，接触力预测误差降低了30%。"
conclusion: STRIDE通过显式分离物理结构与随机残差，成功实现了兼具物理一致性和多模态随机行为表达能力的动力学建模。
---
