...
## 摘要
未知环境中的自主探索通常依赖于机载状态估计来进行定位与建图。现有的探索方法主要侧重于最大化覆盖效率，但往往忽略了视觉惯性里程计（VIO）的性能在很大程度上取决于鲁棒视觉特征的可用性。因此，探索策略可能会将机器人引导至特征稀疏区域，导致跟踪性能下降，进而引发里程计漂移、地图损坏和任务失败。我们为配备双目相机的无人机（UAV）提出了一种分层感知感知探索框架，该框架将探索进度与特征可观测性显式耦合。我们的方法（i）利用全局特征图将每个候选边界与预期的特征质量相关联，并优先选择具有视觉信息量的子目标；（ii）沿规划的运动路径优化连续偏航轨迹，以维持稳定的特征跟踪。我们在具有不同纹理水平的仿真环境以及具有大量无纹理墙壁的真实室内实验中评估了我们的方法。与忽略特征质量和/或不优化连续偏航的基准方法相比，我们的方法保持了更可靠的特征跟踪，减少了里程计漂移，并在里程计误差超过指定阈值前，平均实现了高出 30% 的覆盖率。
