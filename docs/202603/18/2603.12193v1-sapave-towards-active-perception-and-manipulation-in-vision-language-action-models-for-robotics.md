...
tldr: SaPaVe是一个端到端框架，旨在解决机器人交互中的主动感知与操作难题。它通过解耦相机与操作动作，并采用自底向上的训练策略（先学习语义相机控制，再进行联合优化），显著提升了在动态视角下的任务鲁棒性。该研究还引入了ActiveViewPose-200K数据集和ActiveManip-Bench基准，实验证明其在真实场景中的成功率远超现有SOTA模型。
selection_source: fresh_fetch
motivation: 现有方法难以在复杂场景中统一语义驱动的主动感知与鲁棒的、视角无关的操作执行。
method: 提出解耦相机与操作动作的SaPaVe框架，利用3D几何感知模块并采用从语义相机控制到联合优化的分阶段训练策略。
result: "在模拟和真实环境实验中，SaPaVe的成功率比GR00T N1和π0等先进模型高出多达31.25%。"
conclusion: 通过解耦且协调的训练策略实现感知与执行的紧密耦合，是提升机器人主动操作效率和泛化能力的关键。
---
