...
tldr: 针对目标导向视觉导航中全局拓扑规划与局部反应式控制脱节的问题，本文提出 IntentReact 框架。该框架引入“意图”作为紧凑接口，将全局拓扑引导编码为低维方向信号，用于调节航点预测策略。这使得机器人即使在局部观测受限或误导时也能保持拓扑一致性，快速调整航向，在保持反应式控制鲁棒性的同时，显著提升了导航的成功率和执行效率。
selection_source: fresh_fetch
motivation: 现有的拓扑导航方法在局部决策时仅依赖当前视野，导致机器人在局部观测误导下难以利用全局信息进行有效的重定向。
method: 提出一种意图调节的物体中心导航框架，通过将全局拓扑引导编码为低维方向信号（意图），引导局部航点预测策略向拓扑一致的方向偏移。
result: 实验结果表明，IntentReact 在导航成功率和执行质量上均显著优于现有的物体中心导航方法。
conclusion: 通过在全局规划与局部控制间引入意图接口，可以有效弥补局部感知的局限性，实现更高效、鲁棒的物体目标导航。
---
