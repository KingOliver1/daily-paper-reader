...
## 摘要
建图在机器人和自主系统中至关重要，因为它为路径规划提供了空间基础。高效的建图使规划算法能够在确保安全的同时生成可靠路径，并实时适应复杂环境。固定分辨率的建图方法通常会产生过于保守的障碍物表示，导致在拥挤场景中出现次优路径或规划失败。为解决这一问题，我们引入了并行八叉树地图（Parallel OctoMapping, POMP），这是一种基于 OctoMap 的高效建图技术，能够最大化可用的自由空间并支持多线程计算。据我们所知，POMP 是首个在固定占据网格分辨率下，既能细化自由空间表示，又能保持地图保真度并兼容现有基于搜索的规划器的方法。因此，它可以集成到现有的规划流程中，在显著提高计算效率的同时，获得更高的寻路成功率和更短的路径长度，尤其是在拥挤环境中。
