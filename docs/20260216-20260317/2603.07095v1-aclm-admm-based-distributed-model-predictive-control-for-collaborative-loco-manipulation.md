...
conclusion: 该分布式控制方案成功平衡了计算效率与动力学精度，为复杂环境下多足式机器人的协同作业提供了有效途径。
---

## 摘要
通过移动操作协作运输重型负载是足式机器人在复杂、非结构化环境中运行的一项具有挑战性但至关重要的能力。集中式规划方法（如整体轨迹优化）能够捕捉机器人与负载之间的动力学耦合，但其扩展性随系统规模增加而变差，限制了实时应用性。相比之下，分层和完全去中心化的方法通常忽略了力和动力学交互，导致行为过于保守。本研究提出了一种基于交替方向乘子法（ADMM）的分布式模型预测控制框架，用于配备机械臂的四足机器人团队的协作式移动操作。通过利用负载引起的耦合结构，全局最优控制问题被分解为具有一致性约束的并行单机器人级子问题。该分布式规划器以收缩时域方式运行并实现了快速收敛，每个规划周期仅需几次 ADMM 迭代。一个感知力矩的全身控制器执行规划的轨迹，同时跟踪运动和交互力矩。涉及多达四个机器人的广泛仿真证明了该方法的扩展性、实时性能以及对模型不确定性的鲁棒性。
