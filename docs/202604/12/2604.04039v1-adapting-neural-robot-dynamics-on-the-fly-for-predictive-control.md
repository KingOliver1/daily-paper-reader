...
tldr: 针对自主移动机器人预测控制中物理模型精度不足及数据驱动模型训练缓慢的问题，本文提出一种神经动力学模型快速在线适应方法。该方法通过离线学习增量模型并结合在线低秩二阶参数更新，实现了无需完全重训的实时模型调整。在四旋翼无人机上的实验表明，该方法能有效应对新运行环境，显著提升了预测跟踪控制的鲁棒性。
selection_source: fresh_fetch
motivation: 旨在解决物理模型过于简化而传统深度学习模型在线更新速度慢、数据需求量大的矛盾。
method: 结合离线增量神经动力学训练与在线低秩二阶参数自适应技术，实现模型参数的快速迭代。
result: 在真实四旋翼机器人实验中，该方法在未知操作条件下实现了稳健且高精度的预测跟踪控制。
conclusion: 该研究证明了结合离线预训练与高效在线二阶优化是提升机器人动态模型实时适应能力的有效途径。
---
