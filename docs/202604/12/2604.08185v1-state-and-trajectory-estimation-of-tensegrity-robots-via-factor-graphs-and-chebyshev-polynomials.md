...
score: 7.0
evidence: 机器人状态与轨迹估计
tldr: 针对张拉整体机器人非线性及欠约束动力学导致的状态估计难题，本文提出一种基于因子图和切比雪夫多项式的两阶段估计方法。该方法首次将因子图应用于此类机器人，通过融合RGB-D相机与电缆长度传感器数据，并结合马氏距离聚类处理噪声，实现了高精度的连续时间状态与轨迹估计，为闭环控制和系统辨识提供了可靠基础。
selection_source: fresh_fetch
motivation: 张拉整体机器人复杂的非线性动力学和欠约束特性使得传统状态估计方法难以满足闭环控制和机器学习对高精度连续轨迹的需求。
method: 提出一种融合多传感器数据的因子图框架，并利用马氏距离聚类算法处理噪声，结合切比雪夫多项式估计最优速度及中间状态。
result: 在模拟和真实世界数据上的测试表明，该方法在处理复杂运动时具有高保真度，性能显著优于传统的ICP算法。
conclusion: 因子图与切比雪夫多项式的结合为张拉整体机器人提供了鲁棒且连续的状态估计方案，有效解决了其结构特性带来的估计挑战。
---

## 摘要
张拉整体机器人具有顺应性和适应性，但其非线性且欠约束的动力学特性使得状态估计具有挑战性。对所有刚性连杆进行可靠的连续时间估计对于闭环控制、系统辨识和机器学习至关重要；然而，传统方法往往难以胜任。本文提出了一种用于线驱动张拉整体机器人鲁棒状态或轨迹估计（即滤波或平滑）的两阶段方法。针对在线状态估计，本研究引入了一种基于因子图的方法，该方法融合了来自 RGB-D 相机和车载线缆长度传感器的测量数据。据作者所知，这是因子图在该领域的首次应用。因子图是一个自然的选择，因为它们利用了机器人的结构特性，并提供了能够处理实际应用中非线性问题的有效传感器融合方案。与基于 ICP 的算法相比，用于处理噪声的基于马氏距离的聚类算法，以及用于估计最可能速度和中间状态的切比雪夫多项式方法，在仿真和真实世界数据上均表现良好。结果表明，该方法为复杂的张拉整体机器人运动提供了高保真、连续时间的状态和轨迹估计。
