...
tags: ["query:robot"]
score: 6.0
evidence: 具身机器人任务中的6D物体位姿估计视觉模型
tldr: OMNI-PoseX 是一款针对具身智能任务设计的快速 6D 物体位姿估计视觉基础模型。它通过统一开放词汇感知与 SO(3) 感知的反射流匹配预测器，解决了现有方法在开放世界中泛化性差、实时性低的问题。该模型解耦了物体语义理解与几何一致性旋转推理，并采用轻量化多模态融合策略。在多项基准测试和机器人抓取实验中，它展现了 SOTA 级别的精度、实时性以及对未知物体的卓越零样本泛化能力。
selection_source: fresh_fetch
motivation: 现有 6D 位姿估计方法受限于闭集假设和几何无关的回归方案，难以满足具身智能在开放世界中的泛化性、稳定性和实时性需求。
method: 提出一种结合开放词汇感知与 SO(3) 感知反射流匹配的架构，通过轻量化多模态融合将语义嵌入与几何特征解耦，并在大规模数据集上进行训练。
result: 在多个基准测试中达到 SOTA 精度，实现了实时推理效率，并成功应用于复杂场景下的机器人零样本抓取任务。
conclusion: OMNI-PoseX 证明了将开放词汇理解与几何一致性推理相结合是实现稳健、高效且具备泛化能力的具身位姿估计的有效途径。
---
