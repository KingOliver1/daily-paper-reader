...
score: 6.0
evidence: 自主智能体的视觉导航
tldr: 针对具身智能视觉导航中生成式策略（如扩散模型和薛定谔桥）推理速度慢的问题，本文提出Rectified Schrödinger Bridge Matching (RSBM) 框架。该方法通过引入熵正则化参数统一了薛定谔桥与确定性最优传输，利用速度结构不变性实现单网络多强度控制。RSBM在减少采样步数的同时保持了多模态覆盖，仅需3步积分即可实现高性能导航，显著提升了实时控制效率。
selection_source: fresh_fetch
motivation: 现有的生成式导航策略因高方差随机传输需要大量推理步数，难以满足机器人实时控制的低延迟需求。
method: 提出RSBM框架，利用速度结构不变性，通过调节熵正则化参数在多模态分布捕获与路径平滑度之间取得平衡。
result: "在无需蒸馏或多阶段训练的情况下，RSBM仅需3步推理即可达到94%的余弦相似度和92%的导航成功率。"
conclusion: RSBM有效缩小了高保真生成策略与具身智能实时性要求之间的差距，为实现少步数、高性能的视觉导航提供了新方案。
---

## 摘要
视觉导航是具身智能（Embodied AI）中的核心挑战，要求自主智能体将高维感官观测转化为连续、长时程的动作轨迹。虽然基于扩散模型和薛定谔桥（Schrödinger Bridges, SB）的生成式策略能有效捕捉多模态动作分布，但由于高方差的随机传输，它们通常需要数十个积分步，这为实时机器人控制构成了关键障碍。我们提出了修正薛定谔桥匹配（Rectified Schrödinger Bridge Matching, RSBM），该框架利用了标准薛定谔桥（$\varepsilon=1$，最大熵传输）与确定性最优传输（$\varepsilon\to 0$，如条件流匹配）之间共享的速度场结构，并由单一熵正则化参数 $\varepsilon$ 控制。我们证明了两个关键结果：（1）条件速度场的函数形式在整个 $\varepsilon$ 谱范围内保持不变（速度结构不变性），使得单一网络能够胜任所有正则化强度；（2）减小 $\varepsilon$ 会线性降低条件速度方差，从而实现更稳定的粗步长 ODE 积分。RSBM 锚定于一个可缩短传输距离的学习条件先验，在中间值的 $\varepsilon$ 下运行，以平衡多模态覆盖能力和路径直度。实验表明，虽然标准桥需要 10 步以上才能收敛，但 RSBM 在仅 3 个积分步下即可实现超过 94% 的余弦相似度和 92% 的成功率——且无需蒸馏或多阶段训练——显著缩小了高保真生成策略与具身智能低延迟需求之间的差距。
