...
## 摘要
机器人操纵需要理解环境的 3D 空间结构及其时间演化，然而大多数现有策略往往忽略了其中之一或两者。它们通常依赖于 2D 视觉观测以及在静态图像-文本对上预训练的主干网络，导致对数据需求量大，且对环境动力学的理解有限。为了解决这一问题，我们提出了 MV-VDP，这是一种多视角视频扩散策略，能够对环境的 3D 时空状态进行联合建模。其核心思想是同时预测多视角热图视频和 RGB 视频，这 1) 对齐了视频预训练与动作微调的表示格式，2) 不仅指定了机器人应采取的动作，还规定了环境在这些动作响应下预期的演化方式。大量实验表明，MV-VDP 实现了高效利用数据、鲁棒、可泛化且具有可解释性的操纵。仅需十条演示轨迹且无需额外预训练，MV-VDP 即可成功执行复杂的现实世界任务，在各种模型超参数下表现出极强的鲁棒性，能够泛化到分布外设置，并预测逼真的未来视频。在 Meta-World 和真实机器人平台上的实验证明，MV-VDP 的表现始终优于基于视频预测、基于 3D 以及视觉-语言-动作模型，在高效利用数据的多任务操纵领域树立了新的标杆。
