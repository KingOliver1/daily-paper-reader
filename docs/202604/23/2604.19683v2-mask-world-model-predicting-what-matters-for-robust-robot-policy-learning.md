...
tags: ["query:robot"]
score: 7.0
evidence: 用于鲁棒机器人策略学习和接触关系建模的世界模型
...
motivation: 传统的基于RGB视频预测的世界模型容易受到背景和光照等无关视觉因素干扰，导致生成的控制策略在复杂环境下不够稳健。
method: 提出掩码世界模型（MWM），利用视频扩散架构预测语义掩码的演化，并将其与扩散策略头集成以实现端到端控制。
result: 在LIBERO和RLBench仿真基准以及真实世界实验中，MWM显著优于现有的基于RGB的世界模型，并表现出对纹理丢失的高度鲁棒性。
conclusion: 通过预测语义掩码而非像素，MWM能有效过滤视觉噪声并提取关键物理特征，为构建鲁棒的机器人通用策略提供了新路径。
---
