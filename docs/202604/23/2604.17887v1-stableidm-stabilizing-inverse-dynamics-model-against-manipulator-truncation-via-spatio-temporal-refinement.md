...
## 摘要
逆动力学模型 (IDM) 将视觉观测映射为低级动作指令，是具身智能中数据标注和策略执行的核心组件。然而，在机械臂截断（一种使状态恢复变得病态并导致控制不稳定的常见失效模式）下，其性能会严重下降。我们提出了 StableIDM，这是一个通过细化视觉输入特征以在上述部分可观测条件下稳定动作预测的时空框架。StableIDM 集成了三个互补组件：(1) 以机器人为中心的辅助掩码，用于抑制背景杂波；(2) 用于几何感知空间推理的方向特征聚合 (DFA)，沿从可见机械臂推断出的方向提取各向异性特征；(3) 时域动力学细化 (TDR)，通过运动连续性平滑并修正预测。广泛的评估验证了我们的方法：在 AgiBot 基准测试的严重截断情况下，StableIDM 将严格动作准确率提高了 12.1%，并在真实机器人回放中将平均任务成功率提高了 9.7%。此外，在解码视频生成的规划时，它将端到端抓取成功率提升了 11.5%；作为自动标注器时，它将下游 VLA 真实机器人成功率提升了 17.6%。这些结果表明，StableIDM 为具身智能中的策略执行和数据生成提供了一个鲁棒且可扩展的主干网络。
