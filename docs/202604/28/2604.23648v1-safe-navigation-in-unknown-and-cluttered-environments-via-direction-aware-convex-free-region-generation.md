...
tags: ["query:robot"]
score: 6.0
evidence: 用于安全导航和运动规划的凸自由区域生成
tldr: 针对未知复杂环境下的机器人导航，本文提出了一种方向感知的凸自由区域生成框架。该方法将候选运动方向和机器人几何形状融入区域生成过程，解决了传统方法在狭窄通道中难以兼顾通过性的问题。结合基于Lipschitz的连续安全认证和局部轨迹优化，实现了在2D、3D及真实场景（四足机器人、无人机）中的高效、连续无碰撞导航。
selection_source: fresh_fetch
motivation: 现有凸自由区域生成方法主要依赖障碍物几何，忽略了机器人形状和运动方向，导致在复杂环境中难以生成支持后续通行的区域。
method: 提出一种结合运动方向与机器人几何的凸区域生成方法，并利用Lipschitz连续安全认证和局部细化技术确保轨迹的绝对安全。
result: 实验证明该方法生成的自由区域更符合通行需求，在2D/3D仿真及四足机器人、无人机实测中均表现出极高的导航可靠性。
conclusion: 该框架通过方向感知的区域生成和连续安全保障，显著提升了机器人在复杂未知环境中的避障能力和规划效率。
---

## 摘要
凸自由区域为未知且拥挤环境中的机器人导航提供了一种结构化且易于优化的无碰撞空间表示。然而，现有方法通常主要根据周围障碍物的几何形状来扩大局部无碰撞区域。在拥挤的环境中，此类策略可能无法生成既能适应机器人几何形状，又能沿候选运动方向保持可通行延伸的区域，从而限制了后续的遍历，特别是在狭窄通道中。即使存在这样的区域，安全运动生成仍然具有挑战性，因为当显式建模机器人几何形状时，对离散轨迹样本进行安全性检查并不能保证连续的无碰撞运动。为了解决这些问题，我们提出了一个导航框架，将候选运动方向和机器人几何形状共同纳入凸自由区域的生成中，并通过连续安全轨迹生成来实现连续无碰撞运动。在每个区域内，该框架执行几何感知的目标位姿选择和轨迹生成，并结合基于 Lipschitz 的连续安全性认证和局部细化。生成的自由区域和候选运动被维护在一个基于区域的图中，以支持增量式规划。在拥挤的 2D 导航场景中的定量结果表明，所提方法生成的自由区域能更好地与后续遍历对齐，并实现可靠的无碰撞导航；此外，在四足机器人和无人机上进行的 3D 及真实世界实验证明了该框架的可扩展性和实际适用性。开源项目见 https://github.com/ZhichengSong6/FRGraph。
