...
tags: ["query:robot"]
score: 7.0
evidence: 用于获取信息观测的主动感知与路径规划
tldr: 本研究针对移动机器人主动感知中的路径规划问题，提出了一种高效的节点级束搜索（NBS）算法。针对现有方法计算开销大或约束过多的局限，NBS通过在每个节点保留前B个候选路径来增强探索能力。此外，研究引入了期望增益指标以平衡探索与利用，并设计了快速探索随机圆环图（RRAG）以确保复杂环境下的连通性。实验表明，该方法在多项任务中优于现有算法，并在真实机器人平台上得到了验证。
selection_source: fresh_fetch
motivation: 现有的主动感知方法在计算效率和路径约束之间难以平衡，且标准束搜索算法容易陷入局部最优并对参数敏感。
method: 提出了节点级束搜索算法（NBS），并结合期望增益指标和新型随机圆环图（RRAG）来优化路径选择与环境建模。
result: "NBS结合RRAG在三类代表性主动感知任务中均取得最高性能，在部分任务中比现有最先进算法提升了至少20%。"
conclusion: 该研究提供了一种高效且可扩展的主动感知框架，通过改进搜索策略和图构建方法，显著提升了机器人在复杂环境下的信息采集效率。
---

## 摘要
主动感知是自主机器人领域的一个基本问题，机器人必须决定移动到何处以及感知什么，以获取最有信息量的观测结果来完成任务。现有方法要么在启发式选择的信息节点上求解计算代价高昂的旅行商问题，要么采用更高效但约束过多的最短路径树公式。为了解决这些局限性，我们探索了束搜索（beam search）算法作为可扩展的替代方案。虽然标准束搜索通过在每个深度级别保留前 B 条路径来提供可扩展性，但它容易陷入局部最优，并表现出参数敏感性。我们的第一个贡献是节点级束搜索（NBS）算法，该算法在每个节点维护前 B 个候选路径，从而能够更有效地探索解空间。在图上的系统基准测试表明，NBS 始终优于其他基准方法，并且即使在较低的束宽下也能保持强大的性能。作为第二个贡献，我们将前沿（frontiers）的概念整合到路径选择标准中，引入了期望增益指标，与现有替代方案相比，该指标能更好地平衡探索与利用。我们的第三个贡献提出了快速探索随机圆环图（RRAG），这是一种新颖的图构建方法，它保留了完整的朝向采样，并通过后备的局部采样规划器确保在拥挤环境中的连通性。大量实验表明，NBS 与 RRAG 相结合在所有三个代表性主动感知任务中均实现了最高性能，在其中一个或多个任务中比现有最先进算法至少提升了 20%。我们还在不同场景下的真实机器人平台上进一步验证了该方法。
