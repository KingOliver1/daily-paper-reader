...
tldr: 本文提出3D-ALP，一种用于机器人操作的System 2推理引擎。它将蒙特卡洛树搜索（MCTS）与3D一致的世界模型相结合，通过持久的相机到世界锚点解决了反应式策略在物体被遮挡时失效的问题。在需要空间记忆的5步序列任务中，3D-ALP显著提升了成功率，并克服了UCT-MCTS在连续动作空间中的结构性缺陷，为具备长期记忆的机器人规划提供了新方案。
selection_source: fresh_fetch
motivation: 传统的反应式机器人策略仅依赖当前视觉输入，无法处理物体被遮挡或移出视野后的长程空间记忆与规划挑战。
method: 结合MCTS与3D一致的世界模型，利用持久的3D锚点在搜索过程中维持环境状态，实现对不可见目标的精确回溯与规划。
result: 在空间记忆任务中，3D-ALP将需记忆步骤的成功率从0.006提升至0.650，且在第5步任务中表现出极强的鲁棒性。
conclusion: 3D锚定搜索是实现机器人持久空间记忆的核心驱动力，该方法有效解决了MCTS在连续机器人操作任务中的应用瓶颈。
---
