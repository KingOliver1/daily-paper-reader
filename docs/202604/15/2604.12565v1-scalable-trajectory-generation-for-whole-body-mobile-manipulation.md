...
tldr: 针对移动操作机器人在复杂环境中面临的数据获取难题，本文提出了AutoMoMa框架。该框架通过AKR建模统一底盘、机械臂与物体运动学，并利用GPU加速并行轨迹优化，实现了比传统方法快80倍的数据生成速度。研究生成了包含50万条轨迹的大规模数据集，并证明了海量数据是提升模仿学习策略泛化能力的关键，为实现通用的全身移动操作提供了基础设施。
selection_source: fresh_fetch
motivation: 现有的移动操作数据获取方法效率低下，难以满足复杂环境下全身协同控制对大规模多样化数据的需求。
method: 提出AutoMoMa框架，通过AKR建模统一运动学链条，并结合GPU加速的并行轨迹优化技术实现高效数据生成。
result: 实现了每GPU小时生成5000个回合的高效率，并构建了包含50万条物理有效轨迹、涵盖330个场景的大规模数据集。
conclusion: 实验证明数据规模是提升移动操作性能的核心瓶颈，AutoMoMa为大规模全身移动操作研究提供了高效的基础设施。
---

## 摘要
部署在非结构化环境中的机器人必须协调全身运动——即同时移动移动底座和机械臂——以与物理世界进行交互。这种耦合的移动性与灵活性导致状态空间随场景和物体的多样性呈组合式增长，因此需要比固定底座操作规模大得多的数据集。然而，现有的数据获取方法（包括远程操作和规划）要么耗费人力，要么在大规模应用时计算成本过高。核心瓶颈在于缺乏一个可扩展的流水线，用于在多种机器人形态和环境中生成大规模、物理有效且协调的轨迹数据。在此，我们介绍了 AutoMoMa，这是一个 GPU 加速的框架，它将 AKR 建模（将底座、机械臂和物体的运动学整合到单一链中）与并行化轨迹优化相结合。AutoMoMa 每 GPU 小时可生成 5,000 个回合（比基于 CPU 的基准快 80 倍以上），并产出了一个包含超过 50 万条物理有效轨迹的数据集，涵盖 330 个场景、多种关节物体和多种机器人形态。以往的数据集被迫在规模、多样性或运动学保真度之间做出妥协，而 AutoMoMa 同时解决了这三个问题。通过训练下游模仿学习（IL）策略进一步发现，即使是单个关节物体任务，也需要数万次演示才能使最先进（SOTA）方法达到约 80% 的成功率，这证实了数据稀缺——而非算法限制——一直是主要的制约因素。因此，AutoMoMa 桥接了高性能规划与可靠的基于 IL 的控制，为协调移动操作研究提供了此前缺失的基础设施。通过使大规模、运动学有效的训练数据变得实用，AutoMoMa 展示了能够在现实世界多样且非结构化设置中运行的可泛化全身机器人策略。
