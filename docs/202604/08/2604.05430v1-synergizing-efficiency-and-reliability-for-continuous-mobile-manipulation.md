...
tags: ["query:robot"]
score: 6.0
evidence: 移动操作的可靠性感知轨迹规划器
tldr: 针对移动操作中效率与可靠性难以兼得的问题，本文提出一种协同框架。该框架包含一个可靠性感知轨迹规划器，通过时空优化生成高效且稳健的全局轨迹；配合相位切换控制器，在全局跟踪与任务误差补偿间无缝切换。实验证明，该方法在存在不确定性的连续任务中，显著提升了作业效率和成功率。
selection_source: fresh_fetch
motivation: 解决移动机器人连续操作中长时程规划与实时反应之间的冲突，以及追求效率时对感知稳定性和可靠性的负面影响。
method: 提出由可靠性感知轨迹规划器、相位切换控制器和分层初始化机制组成的统一框架，实现高效且稳健的运动控制。
result: "在真实环境下的连续任务评估中，该方法在保持最高效率的同时，将任务成功率提升了26.67%至81.67%。"
conclusion: 该框架成功实现了移动操作中效率与可靠性的协同，并展现出对动态干扰和多样化末端约束的良好泛化能力。
---

## 摘要
人类能够无缝融合前瞻性规划与即时反馈，从而不间断地执行连续移动操作任务，实现高效率与高可靠性。在机器人上复制这种流畅且可靠的行为仍然具有根本性的挑战，这不仅源于长时程规划与实时反应性之间的冲突，还因为在不确定环境中过度追求效率会损害可靠性：它会削弱稳定的感知能力和补偿潜力，同时增加意外接触的风险。在这项工作中，我们提出了一个统一框架，旨在协同连续移动操作中的效率与可靠性。该框架包含一个可靠性感知轨迹规划器，将可靠执行的关键要素嵌入到时空优化中，从而生成高效且具有可靠性保障的全局轨迹。此外，该框架还配备了一个基于阶段的切换控制器，可在追求效率的全局轨迹跟踪与追求可靠性的任务误差补偿之间无缝切换。我们还研究了一种分层初始化方法，以应对复杂长时程规划问题中的在线重规划。真实世界评估表明，我们的方法能够在存在不确定性（如动态干扰、感知和控制误差）的情况下，高效且可靠地完成连续任务。此外，该框架可推广至具有多种末端执行器约束的任务。与最先进的基准方法相比，我们的方法在保持最高效率的同时，将任务成功率提高了 26.67%--81.67%。全面的消融实验进一步验证了各组件的贡献。源代码将会开源。
