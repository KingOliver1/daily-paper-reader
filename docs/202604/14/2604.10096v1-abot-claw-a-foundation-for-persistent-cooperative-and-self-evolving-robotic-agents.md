...
## 摘要
当前的具身智能系统在开放世界环境中，高层推理与底层物理执行之间仍存在显著鸿沟。尽管视觉-语言-动作（VLA）模型提供了强大的感知和直观响应，但其开环特性限制了长程任务的表现。引入系统 2 认知机制的智能体虽然改善了规划，但通常在具有预定义工具集且真实系统控制受限的封闭沙盒中运行。OpenClaw 提供了一个具有完整系统权限的本地化运行时，但缺乏长时程、多机器人执行所需的具身控制架构。因此，我们提出了 ABot-Claw，这是 OpenClaw 的具身扩展，它集成了：1) 具有能力驱动调度的统一具身接口，用于异构机器人协作；2) 以视觉为中心的跨具身多模态记忆，用于持久化上下文保留和具身化检索；3) 基于评论家的闭环反馈机制，配备通用奖励模型，用于在线进度评估、局部修正和重新规划。通过跨越 OpenClaw 层、共享服务层和机器人具身层的解耦架构，ABot-Claw 实现了真实世界的交互，完成了从自然语言意图到物理动作的闭环，并支持在开放、动态环境中逐步自进化的机器人智能体。
