...
title_zh: Ψ-Map：全景表面集成建图助力 Real2Sim 迁移
...
## 摘要
开放词汇全景重建对于高级机器人感知和仿真至关重要。然而，现有基于 3D 高斯泼溅（3DGS）的方法在处理大规模场景时，往往难以同时兼顾几何精度、连贯的全景理解以及实时推理频率。在本文中，我们提出了一个综合框架，集成了几何增强、端到端全景学习和高效渲染。首先，为了确保大规模环境中的物理真实性，我们利用 LiDAR 数据构建了平面约束的多模态高斯混合模型（GMM），并采用 2D 高斯面元（surfels）作为地图表示，从而实现了高精度的表面对齐和连续的几何监督。在此基础上，为了克服传统多阶段全景分割流水线中固有的误差累积和繁琐的跨帧关联问题，我们设计了一种查询引导的端到端学习架构。通过在视锥体内利用局部交叉注意力机制，该系统将 2D 掩码特征直接提升到 3D 空间，实现了全局一致的全景理解。最后，针对高维语义特征带来的计算瓶颈，我们引入了精确分块相交（Precise Tile Intersection）和 Top-K 硬选择策略来优化渲染流水线。实验结果表明，我们的系统在大规模场景中实现了卓越的几何和全景重建质量，同时保持了超过 40 FPS 的推理速率，满足了机器人控制回路的实时性要求。
