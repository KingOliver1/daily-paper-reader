...
tldr: 本文提出一种针对具有局部未知输入的离散时间大规模系统的分布式未知输入观测器（DUIO）框架。该框架通过让各节点先利用局部数据估计部分可重构状态，再利用分布式优化算法进行全局信息融合，实现了对系统状态的准确估计。研究不仅证明了估计误差的有界性，还通过引入归一化步骤解决了系统病态几何条件下的收敛问题，为分布式状态估计提供了高效且鲁棒的解决方案。
selection_source: fresh_fetch
motivation: 旨在解决大规模系统中由于传感器空间分布和局部未知输入干扰导致的状态估计难题。
method: 采用局部状态重构结合分布式优化算法，并嵌入归一化步骤以优化系统几何条件。
result: 理论分析给出了显式的误差界限，仿真结果验证了该框架在处理未知输入和提升估计精度方面的有效性。
conclusion: 该方法通过优化手段有效融合了分布式节点的局部估计，是处理复杂环境下大规模系统状态估计的可靠方案。
---
