...
motivation: 旨在解决具身智能体在长程导航任务中每一步都进行推理导致的计算效率低下问题。
method: 引入基于动作熵的自适应推理机制，仅在关键决策点激活推理，并结合混合监督微调和在线强化学习进行训练。
result: 在 CHORES-S ObjectNav 基准测试中，HiRO-Nav 在成功率和 Token 效率之间取得了比全推理或无推理基准更好的平衡。
conclusion: 通过自适应地在反射性动作和深思熟虑的推理之间切换，可以显著提升具身导航的效率和决策质量。
---
