...
tags: ["query:robot"]
score: 7.0
evidence: 导航中的路径规划与约束最优控制
tldr: 本研究针对复杂环境下的自主系统安全导航问题，提出了一种结合神经辐射场（NeRF）与可达集（Reachable Sets）的新方法。利用NeRF对障碍物和自身进行体积建模，并结合可达集表示机器人在状态空间中的实时安全能力。通过将路径规划转化为包含线性矩阵不等式（LMI）约束的最优控制问题，实现了在复杂障碍物环境中的安全避障。仿真结果验证了该方法在多种场景下的有效性。
selection_source: fresh_fetch
motivation: 旨在解决自主机器人在充满各种几何形状障碍物的复杂环境中实现安全导航的挑战。
method: 结合NeRF的体积表示能力与可达集理论，通过线性矩阵不等式约束的受限最优控制进行路径规划。
result: 在两种包含大量障碍物的仿真场景中成功演示了路径规划过程，验证了安全导航能力。
conclusion: 证明了利用可达集结合NeRF表示在处理受限最优控制问题中实现安全导航的可行性与有效性。
---
