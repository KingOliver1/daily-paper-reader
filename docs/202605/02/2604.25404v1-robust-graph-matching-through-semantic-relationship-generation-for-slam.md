...
tldr: 针对室内SLAM中重复或对称布局导致的定位歧义问题，本文提出一种增强语义的图匹配方法。该方法通过显式建模检测到的物体与房间、墙面等结构元素之间的语义关系，在几何验证前过滤候选对应关系。实验表明，该方法在iS-Graphs框架下显著降低了搜索复杂度，提高了对称场景下的定位鲁棒性和计算效率。
selection_source: fresh_fetch
motivation: 传统的基于结构的图匹配在处理室内重复或对称布局时，仅靠结构线索难以消除定位歧义。
method: 提出一种语义增强的图匹配方法，通过建模物体与结构元素（如房间、墙面）间的关系来预过滤候选匹配。
result: 实验结果显示，该方法显著减少了候选匹配数量，提高了计算效率，并在对称场景中实现了更快的收敛。
conclusion: 结合语义关系的图匹配能有效解决室内SLAM中的环境对称性挑战，提升定位的鲁棒性与效率。
---
