...
## 摘要
当前的视觉-语言-动作（VLA）模型主要关注将2D观测映射到动作，但在时空感知和推理方面表现出显著局限性：1) 空间表示通常依赖于额外的传感器，引入了巨大的计算开销；2) 视觉推理通常局限于未来帧预测，缺乏与基于指令的场景对齐，从而损害了时空一致性。为应对这些挑战，我们提出了 ConsisVLA-4D，这是一个统一且高效的框架，旨在增强3D感知和4D推理中的时空一致性。具体而言，我们设计了：1) CV-Aligner，通过过滤指令相关区域并对齐多个视角下的物体身份，确保跨视图的物体语义一致性；2) CO-Fuser，通过使用紧凑的潜表示消除跨视图物体间的空间关系歧义，保证跨物体的空间几何一致性。在此基础上，我们引入了 3) CS-Thinker，以在动作展开时实现跨场景的时空一致性。它从 CV-Aligner 的物体语义 token 中学习局部动力学的隐式知识，并从 CO-Fuser 的几何 token 中学习全局深度，从而增强了场景变化下的高效视觉推理。大量实验表明，得益于其高效的时空一致性设计，ConsisVLA-4D 在 LIBERO 基准测试和真实世界平台上，相比 OpenVLA 分别实现了 21.6% 和 41.5% 的性能提升，以及 2.3 倍和 2.4 倍的推理加速。ConsisVLA-4D 已开源并公开发布。
